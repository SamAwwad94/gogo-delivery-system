...
class WithdrawalRequestScreen extends StatefulWidget {
  ...
  WithdrawalRequestScreen({this.status});
...
class _WithdrawalRequestScreenState extends State<WithdrawalRequestScreen> {
  ...
  getWithdrawListApiCall() async {
    ...
    await getWithdrawList(page: currentPage, perPage: perPage, status: widget.status).then((value) {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
          appBar: appBarWidget(language.withdrawRequest),
          body: Observer(
            builder: (_) => Stack(
              children: [
                ListView.builder(
                                  ...
                                  if (mData.status!.toString() != DECLINE && mData.status!.toString() != APPROVED)
                                    Row(
                                      children: [
                                        ...
                                        SizedBox(width: 8),
                                        outlineActionIcon(context, Icons.close, Colors.red, () {
                                          ...
                                          children: [
                                            ...
                                            Row(
                                              ...
                                              children: [
                                                ...
                                                mData.status! == REQUESTED ? Text(mData.walletBalance.toString(), style: boldTextStyle(size: 15)) : Text('-', style: boldTextStyle(size: 15)),
                                              ],
                                            ),
                                            Divider(thickness: 0.9, height: 20),
                                            Row(
                                            ...
                                            Row(
                                              ...
                                              children: [
                                                ...
                                                mData.status! == REQUESTED
                                                    ? outlineActionIcon(context, Icons.visibility, appStore.isDarkMode ? white : primaryColor, () {
