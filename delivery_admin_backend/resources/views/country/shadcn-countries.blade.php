@extends('layouts.app')
@section('title', __('message.country'))
@section('content')
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <h5 class="font-weight-bold">{{ $pageTitle ?? trans('message.country') }}</h5>
                            <div class="d-flex flex-wrap gap-2">
                                @if($button ?? '')
                                    {!! $button !!}
                                @endif
                                <a href="{{ route('country.index', ['classic' => 1]) }}" class="shadcn-button shadcn-button-outline">
                        ...
                        <div class="p-3 border-bottom">
                            <form action="{{ route('refactored-country.index') }}" method="GET" class="shadcn-filters">
                                <div class="row">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        <label for="distance_type" class="form-label">{{ __('message.distance_type') }}</label>
                                        <select name="distance_type" id="distance_type" class="form-control shadcn-select">
                                            <option value="">{{ __('message.all') }}</option>
                                            <option value="km" {{ request('distance_type') == 'km' ? 'selected' : '' }}>{{ __('message.km') }}</option>
                                            <option value="mile" {{ request('distance_type') == 'mile' ? 'selected' : '' }}>{{ __('message.mile') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                ...
                                <div class="d-flex justify-content-end">
                                    ...
                                    <a href="{{ route('refactored-country.index') }}" class="shadcn-button shadcn-button-outline">
                            ...
                            <table class="table shadcn-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('message.distance_type') }}</th>
                                ...
                                <tbody>
                                    @if($countries->count() > 0)
                                        @foreach($countries as $country)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" class="custom-control-input select_checkbox" id="select_{{ $country->id }}" data-id="{{ $country->id }}">
                                                        <label class="custom-control-label" for="select_{{ $country->id }}"></label>
                                                    </div>
                                                </td>
                                                @endif
                                                <td>{{ $country->id }}</td>
                                                <td>{{ $country->name }}</td>
                                                <td>{{ $country->code }}</td>
                                                <td>{{ $country->distance_type }}</td>
                                                <td>
                                                    <span class="badge badge-{{ $country->status ? 'success' : 'danger' }}">
                                                        {{ $country->status ? __('message.active') : __('message.inactive') }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        @if(auth()->user()->can('country-edit'))
                                                            <a href="{{ route('refactored-country.edit', $country->id) }}" class="shadcn-button shadcn-button-sm shadcn-button-secondary">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                                            </a>
                                                        @endif
                                                        @if(auth()->user()->can('country-delete'))
                                                            <form action="{{ route('refactored-country.destroy', $country->id) }}" method="POST" class="d-inline">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="shadcn-button shadcn-button-sm shadcn-button-destructive" onclick="return confirm('{{ __('message.delete_form_message', ['form' => __('message.country')]) }}')">
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="{{ $multi_checkbox_delete ? '7' : '6' }}" class="text-center">{{ __('message.no_data_found') }}</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
