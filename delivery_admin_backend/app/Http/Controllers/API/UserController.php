...
class UserController extends Controller
{
    ...
    public function newlogin(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $decryptedEmail, 'password' => $decryptedPassword])) {
            ...
            return json_message_response($message, 401);
        }
    }
    ...
    public function newregister(Request $request)
    {
            ...
            'username'  => [
                ...
                function ($attribute, $value, $fail) {
                    if (filter_var($value, FILTER_VALIDATE_EMAIL)) {
                        return; // Skip unique validation if username is an email
                    }
        ...
        if ($validator->fails()) {
            return json_custom_response([
                'message' => $validator->errors()->first(), // Show only the first validation error message
                'success' => false
            ], 422);
        }
