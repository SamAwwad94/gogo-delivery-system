...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Clone the query to avoid affecting other queries
        $recentOrderQuery = clone $ordersQuery;
        $recent_order = $recentOrderQuery->whereDate('date', '<=', Carbon::now()->format('Y-m-d'))->whereNotIn('status', ['pending'])->orderBy('date', 'desc')->paginate(10);
        ...
        $cityOrders = $ordersQuery->selectRaw('city_id,
            ...
            ->groupBy('city_id')
            ->orderBy('total', 'desc') // Order by total count instead of date
            ->get()
            ->keyBy('city_id');
    ...
    /**
     * Display a dashboard with ShadCN styling.
     *
    ...
    public function shadcnIndex(Request $request)
    {
        ...
        // Get the same data as the regular dashboard
        $ordersQuery = Order::myOrder();
        if ($params['from_date'] && $params['to_date']) {
            if ($params['from_date'] == $params['to_date']) {
                $ordersQuery->whereDate('created_at', '=', $params['from_date']);
        ...
        // Process queries with date filters
        if ($params['from_date'] && $params['to_date']) {
            if ($params['from_date'] == $params['to_date']) {
                $userQuery->whereDate('created_at', '=', $params['from_date']);
                $deliverymanQuery->whereDate('created_at', '=', $params['from_date']);
        ...
        if (!empty($params['from_date']) && !empty($params['to_date'])) {
            $month_start = Carbon::parse($params['from_date']);
            $today = Carbon::parse($params['to_date']);
        } else {
            $month_start = Carbon::now()->startOfMonth();
            $today = Carbon::now();
        }
        ...
        // Get monthly cancelled payment data
        ...
        for ($i = 0; $i < $diff; $i++) {
            ...
            // Order count
            $total = $order_collection->filter(function ($value) use ($date) {
            ...
            // Payment data for completed orders
            $total_amount = $payment_collection->filter(function ($value) use ($date) {
                return $value['date'] == $date;
            })->sum('total_amount');
            ...
            // Payment data for cancelled orders
            $cancel_charges = $payment_cancelled_collection->filter(function ($value) use ($date) {
                return $value['date'] == $date;
            })->sum('cancel_charges');
        ...
        // Get weekly data
        if (!empty($params['from_date']) && !empty($params['to_date'])) {
            $week_start = Carbon::parse($params['from_date']);
            $week_end = Carbon::parse($params['to_date']);
        } else {
            $sunday = strtotime('sunday -1 week');
            $sunday = date('w', $sunday) === date('w') ? $sunday + 7 * 86400 : $sunday;
            $saturday = strtotime(date('Y-m-d', $sunday) . ' +6 days');
        ...
        $weekly_order_count = Order::myOrder()->selectRaw('DATE_FORMAT(created_at , "%w") as days , DATE_FORMAT(created_at , "%Y-%m-%d") as date')
            ->whereBetween('created_at', [$week_start, $week_end])
            ->get()->toArray();
        ...
        // Return the ShadCN dashboard view
        return view('dashboards.admin-dashboard', compact('auth_user', 'data', 'weekly_count', 'cityData', 'cityList', 'params'));
    }
