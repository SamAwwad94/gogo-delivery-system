@foreach($items as $item)
    ...
    <li @lm_attrs($item) @if($item->hasChildren()) @endif @lm_endattrs>
        @if($item->link) <a @lm_attrs($item->link)
                @if($item->hasChildren()) data-toggle="collapse" role="button"
                    aria-expanded="{{ $active != '' ? 'true' : 'false' }}" data-target="#{!! str_replace('#', '', $item->url()) !!}"
                    @else class="nav-link"
                @endif @lm_endattrs href="{!! $item->hasChildren() ? '#' : $item->url() !!}">
                {!! $item->title !!}
                @if($item->hasChildren())
