...
@if($delete_at != null)
    <div class="d-flex justify-content-end align-items-center">
        @if($auth_user->can('order-edit'))
            <a class="mr-2" href="{{ route('order.restore', ['id' => $id, 'type' => 'restore']) }}"
                data--confirmation--restore="true" title="{{ __('message.restore_title') }}"><i class="ri-refresh-line"
        ...
        @if($auth_user->can('order-delete'))
            <a class="mr-2 text-danger" href="javascript:void(0)" data--submit="order{{ $id }}" data--confirmation="true"
