...
(function (jQuery) {
    ...
    jQuery(document).ready(function () {
        ...
        // Handle submenu item clicks (like "All Order") - Updated for Alpine.js
        jQuery(document).on("click", ".sidebar-layout a", function (e) {
            // Keep the parent menu open/active
            var parentMenu = jQuery(this).closest(".submenu").parent();
            if (parentMenu.length) {
                parentMenu.addClass("active");
            }

            // Add active class to the clicked sidebar item
            jQuery(".sidebar-layout").removeClass("active");
            jQuery(this).closest(".sidebar-layout").addClass("active");
        });

        // Active menu - Works with both Bootstrap and Alpine.js
       ...
       -----------------------------------------------------------------------*/
        ...
        const sidebarScrollbarElement =
            document.querySelector(".data-scrollbar"); // The element that has the scrollbar

        if (sidebarScrollbarElement) {
            const sidebarScrollbarInstance = Scrollbar.init(
                sidebarScrollbarElement,
                {
            ...
            if (sidebarScrollbarInstance) {
                const $mainMenu = jQuery("#mm-sidebar-toggle"); // The main UL for MetisMenu

                // Listen for MetisMenu events if MetisMenu is used
                if (typeof $mainMenu.metisMenu === "function") {
                    $mainMenu.on(
                        "shown.metisMenu hidden.metisMenu",
                        function () {
                            sidebarScrollbarInstance.update();
                        }
                    );
                }

                // Listen for Alpine.js transitions on submenus within the scrollable area
                // Update scrollbar when Alpine.js shows/hides submenus
                jQuery(sidebarScrollbarElement)
                    .find(".submenu")
    ...
    $(document).on(
        "click",
        '[data-toggel-extra="side-nav-close"]',
        function () {
            const pannel = $(this).attr("data-expand-extra");
    ...
    $("emoji-picker").on("emoji-click", function (e) {
        $(e.target.dataset.targetInput).val(
            $(e.target.dataset.targetInput).val() + e.detail.unicode
        );
    });
