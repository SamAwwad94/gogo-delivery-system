...
class CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  getStaticDetailsForOrder() async {
    await getCreateOrderDetails(getIntAsync(CITY_ID)).then((value) async {
      ...
      print("------------vehicleList${vehicleList.first.vehicleImage.toString()}");
      ...
      appStore.setIsBiddingStatus(value.appSettingDetail!.isBiddingEnabled.toString());
  ...
  getTotalForOrder() async {
    ...
    Map request = {
      ...
      "insurance_amount": insuranceAmountController.text.isEmpty ? 0 : insuranceAmountController.text
    };
  ...
  double calculateTotalAmount() {
    ...
    if (selectedCoupon?.valueType == "fixed") {
      ...
      double finalTotal = (totalAmount - couponAmount).clamp(0.00, double.infinity);
      ...
      double finalAmount = (totalAmount - discountAmount).clamp(0.00, double.infinity);
  ...
  createOrderApiCall(String orderStatus) async {
    ...
    Map req = {
      ...
      if (selectedCoupon?.couponCode != null && isAppliedCoupon == true) "discount_amount": (totalAmountResponse!.totalAmount! - calculateTotalAmount()),
    ...
    await createOrder(req).then((value) async {
      ...
      if (isSelected == 2) {
        ...
        if (appStore.availableBal > (totalAmountResponse!.totalAmount!)) {
          savePaymentApiCall(paymentType: PAYMENT_TYPE_WALLET, paymentStatus: PAYMENT_PAID, totalAmount: (calculateTotalAmount()).toString(), orderID: value.orderId.toString());
        }
      } else {
  ...
  setPolylines() async {
    print("setPolyline");
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(googleApiKey: googleMapAPIKey, request: PolylineRequest(origin: PointLatLng(pickLat.toDouble(), pickLong.toDouble()), destination: PointLatLng(deliverLat.toDouble(), deliverLong.toDouble()), mode: TravelMode.driving));
      ...
      return Column(
        ...
        children: [
          ...
          Row(
            ...
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              enableBidOptionWidget(context, biddingSelectedOption, Icons.handshake_outlined, language.bids).onTap(() {
            ...
            children: [
              ...
              Container(
                decoration: boxDecorationWithRoundedCorners(
                  borderRadius: BorderRadius.circular(defaultRadius),
                  border: Border.all(color: ColorUtils.colorPrimary.withOpacity(0.2)),
                  backgroundColor: isAppliedCoupon ? Colors.grey.withOpacity(0.5) : Colors.transparent,
                ),
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 4),
                              Text(
                                selectedCoupon?.valueType == "fixed" ? "${language.save} ${appStore.currencySymbol}${selectedCoupon?.discountAmount} ${language.onThisOrder}" : "${language.save} ${selectedCoupon?.discountAmount}% ${language.onThisOrder}",
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                        ...
                        TextButton(
                          ...
                          child: Text(
                            ...
                            style: boldTextStyle(
                              color: isAppliedCoupon ? darkRed : ColorUtils.colorPrimary,
                            ),
                          ),
                        ),
                      ],
                    ...
                    Divider(color: Colors.grey.withOpacity(0.3), height: 0.5).visible(selectedCoupon == null ? false : true),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text("${language.moreCoupons}", style: primaryTextStyle(size: 16)),
                        Icon(
                          Icons.navigate_next,
                          size: 18,
                        )
                      ],
                    ).paddingAll(8).visible(selectedCoupon == null ? false : true).onTap(isAppliedCoupon
                        ? null
                        : () {
                            CouponListScreen().launch(context).then((result) {
                              if (result != null) {
          ...
          Column(
            ...
            children: [
              Text(
                text,
                style: primaryTextStyle(
                  color: insuranceSelectedOption == value ? Colors.white : ColorUtils.themeColor,
                ),
              ),
