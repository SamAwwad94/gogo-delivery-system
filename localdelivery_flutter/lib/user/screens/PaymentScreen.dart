...
class PaymentScreen extends StatefulWidget {
  ...
  PaymentScreen({required this.totalAmount, this.orderId, this.isWallet = false, this.isOnline = false});
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void flutterWaveCheckout() async {
    final customer =
        Customer(name: getStringAsync(NAME), phoneNumber: getStringAsync(USER_CONTACT_NUMBER), email: getStringAsync(USER_EMAIL));
    ...
    if (response.status == 'successful') {
      ...
      if (widget.isWallet == true) {
        ...
        savePaymentApiCall(
            paymentStatus: PAYMENT_PAID, txnId: response.transactionId, paymentType: PAYMENT_TYPE_FLUTTERWAVE, transactionDetail: req);
      }
    ...
    PayWithPayStack().now(
        ...
        transactionCompleted: (paymentData) {
          ...
          if (paymentData.status == true) {
            if (widget.isWallet == true) {
              paymentConfirm(paymentType: PAYMENT_TYPE_PAYSTACK, transactionId: paymentData.reference);
            } else {
              savePaymentApiCall(
                  paymentType: PAYMENT_TYPE_PAYSTACK, paymentStatus: PAYMENT_PAID, transactionDetail: req, txnId: paymentData.reference);
            }
          } else {
  ...
  void payPalPayment() async {
    ...
    if (result != null) {
      ...
      if (widget.isWallet == true) {
        paymentConfirm(paymentType: PAYMENT_TYPE_PAYPAL, transactionId: result.paymentMethodNonce.nonce);
      } else {
        savePaymentApiCall(
            paymentType: PAYMENT_TYPE_PAYPAL,
            paymentStatus: PAYMENT_PAID,
        ...
        if (event["status"] == "success") {
          ...
          if (transactionDetails["isSuccess"]) {
            ...
            if (widget.isWallet == true) {
              ...
              savePaymentApiCall(
                  txnId: transactionDetails['transactionReference'], paymentType: PAYMENT_TYPE_PAYTABS, paymentStatus: 'paid');
            }
          } else {
            toast(language.transactionFailed);
          }
                                ...
                                return GestureDetector(
                                  child: Container(
                                    ...
                                    decoration: boxDecorationWithRoundedCorners(
                                      ...
                                      border: Border.all(
                                          color: mData.type == selectedPaymentType ? ColorUtils.colorPrimary : ColorUtils.borderColor),
                                    ),
                                    child: Row(
                                      children: [
