...
class AuthServices {
      ...
      createAuthUser(email, password).then((user) async {
        if (user != null) {
          ...
          await userService.addDocumentWithCustomId(user.uid, userModel.toJson()).then((value) async {
            Map request = {"email": Encryption.instance.encrypt(userModel.email!), "password": Encryption.instance.encrypt(password!)};
            await logInApi(request).then((res) async {
              ...
              await signInWithEmailPassword(context, email: email.validate(), password: password.validate()).then((value) {
                ...
                if (res.data!.emailVerifiedAt.isEmptyOrNull ||
                  ...
                  print("VerificationListScreen()");
                  VerificationListScreen().launch(context);
                } else {
                  print("UserCitySelectScreen()");
  ...
  //   }
  //
  //   String safeName = (firstName != null && firstName.trim().isNotEmpty) ? firstName.trim() : "Demo";
  //
  //   // 🚀 Log Original & Encrypted Values for Debugging
  //   log("Original Values:");
  //   log("Email: ${currentUser.email!.trim()}");
  //   log("Username: ${currentUser.email!.trim()}");
  //   log("Name: ${firstName?.trim()}");
  //   log("Login Type: $loginType");
  //   log("User Type: $userType");
  //   log("Access Token: $accessToken");
  //   log("Phone Number: ${currentUser.phoneNumber ?? "0000000000"}");
  //
  //   Map req = {
  //     "email": Encryption.instance.encrypt(currentUser.email!.trim()),
  //     "username": Encryption.instance.encrypt(currentUser.email!.trim()),
  //     "name": Encryption.instance.encrypt(safeName),
  //     "login_type": Encryption.instance.encrypt(loginType),
  //     "user_type": Encryption.instance.encrypt(userType),
  //     "accessToken": Encryption.instance.encrypt(accessToken!),
  ...
  //     //   'contact_number': Encryption.instance
  //     //       .encrypt(currentUser.phoneNumber ?? "0000000000"),
  //   };
  //
  //   log("FROM FIREBASE::::: ${req}");
  //
  ...
  Future<void> loginFromFirebase(User currentUser, String loginType, String? accessToken, {String userType = CLIENT}) async {
    ...
    String safeName = (firstName != null && firstName.trim().isNotEmpty) ? firstName.trim() : "User";

    log("safeName   ${Encryption.instance.encrypt(accessToken!)}");
  ...
  Future<void> signInWithGoogle({String userType = CLIENT}) async {
    ...
    if (googleSignInAccount != null) {
      ...
      print("AuthCredential " + user.uid.toString());
