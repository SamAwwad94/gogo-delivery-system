  ...
  Future<void> getUserData() async {
    ...
    await getUserDetail(getIntAsync(USER_ID)).then((value) async {
      ...
      if (data!.emailVerifiedAt.isEmptyOrNull) {
        VerificationStatus status = data!.emailVerifiedAt.isEmptyOrNull ? VerificationStatus.pending : VerificationStatus.completed;
      ...
      if (data!.otpVerifyAt.isEmptyOrNull) {
        VerificationStatus status = data!.otpVerifyAt.isEmptyOrNull ? VerificationStatus.pending : VerificationStatus.completed;
        verificationSteps.add(
          VerificationStep(
              ...
              fun: () {
                if (status == VerificationStatus.pending) {
                  VerificationScreen().launch(context, pageRouteAnimation: PageRouteAnimation.Slide).then((value) => getUserData());
                }
              }),
        );
        // setState(() {});
      ...
      if (data!.documentVerifiedAt.isEmptyOrNull && data?.userType.validate() == DELIVERY_MAN) {
        VerificationStatus status = data!.documentVerifiedAt.isEmptyOrNull ? VerificationStatus.pending : VerificationStatus.completed;
        verificationSteps.add(
          VerificationStep(
              title: language.documentVerification,
              description: language.uploadYourDocument,
              image: Icons.newspaper_rounded,
              status: data!.documentVerifiedAt.isEmptyOrNull ? VerificationStatus.pending : VerificationStatus.completed,
              fun: () {
                if (status == VerificationStatus.pending) {
                  VerifyDeliveryPersonScreen().launch(context, pageRouteAnimation: PageRouteAnimation.Slide).then((value) => getUserData());
                }
              }),
        );
      }
...
class VerificationStep {
  ...
  VerificationStep({required this.title, required this.image, required this.description, required this.status, required this.fun});
}
